Shader "Custom/SkyBoxTest"
{
    ...
    _CloudTiling("Cloud Tiling", Float) = 1.0
    [HDR] _CloudColor("Cloud Color", Color) = (1,1,1,1) // Tambah warna awan
    _CloudRotationSpeed("Cloud Rotation Speed", Float) = 0.05 // Kecepatan rotasi awan
    // _CloudHeight property removed as per user request for direct texture mapping
}
...
SubShader {
    ...
    Pass {
        ...
        float _CloudOpacity;
        float _CloudTiling;
        half4 _CloudColor; // Deklarasi warna awan
        float _CloudRotationSpeed; // Deklarasi kecepatan rotasi
        ...
        v2f vert (appdata_t v)
        {
            ...
            else
            {
                ...
                cOut = clamp(attenuate, 0.0, 1.0);
                    ...
                    OUT.vertex          = -eyeRay;
                #elif SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
                    half3 tempRayDir = -eyeRay; // Store it once
                    OUT.rayDir          = tempRayDir;
                #else
                    // If SKYBOX_SUNDISK_NONE, IN.vertex.xyz is still available.
                    half tempSkyGroundFactor = -eyeRay.y / SKY_GROUND_THRESHOLD; // Store it once
                    OUT.skyGroundFactor = tempSkyGroundFactor;
                #endif
                OUT.vertex.xyz = -eyeRay; // Ensure ray is always passed for cloud UV calculation regardless of SUNDISK mode
                ...
                half4 frag (v2f IN) : SV_Target
                {
                    ...
                    half3 ray = normalize(IN.vertex.xyz); // IN.vertex.xyz contains the normalized eyeRay from vert
                #else
                    half3 ray = normalize(IN.vertex.xyz); // Re-normalize view direction for all paths for clarity in cloud UV mapping
                #endif

                    half y = ray.y / SKY_GROUND_THRESHOLD; // Keep original y calculation for sky/ground blend
                ...
                // --- Penambahan Awan (Sederhana) Dimulai ---
                // Dapatkan waktu untuk animasi rotasi
                float rotationAngle = _Time.y * _CloudRotationSpeed; // _Time.y adalah waktu global, _CloudRotationSpeed mengontrol kecepatan

                // Buat vektor ray XZ untuk rotasi
                float2 rayXZ = ray.xz;
                ...
                float2 rotatedRayXZ;
                rotatedRayXZ.x = rayXZ.x * cosAngle - rayXZ.y * sinAngle;
                rotatedRayXZ.y = rayXZ.x * sinAngle + rayXZ.y * cosAngle;

                // Hitung UV menggunakan rayXZ yang sudah dirotasi
                // Perhatikan bahwa atan2 mengambil (y, x), jadi kita pakai rotatedRayXZ.y, rotatedRayXZ.x
                float u = atan2(rotatedRayXZ.y, rotatedRayXZ.x) * 0.159154943f + 0.5f; // 0.159... = 1 / (2 * PI)
                float v = (ray.y + 1.0) * 0.5f;

                float2 cloudUV = float2(u, v) * _CloudTiling;

                // Sampel tekstur awan
                half4 cloudSample = tex2D(_CloudTex, cloudUV);

                // MODIFIKASI INI: Menggabungkan warna awan secara additive
                // Hitung kontribusi warna awan yang sudah diberi warna dasar (_CloudColor)
                half3 cloudContribution = cloudSample.rgb * _CloudColor.rgb;

                // Kemudian tambahkan kontribusi awan ke warna langit yang sudah ada.
                // cloudSample.a digunakan sebagai alpha untuk mengatur seberapa banyak awan terlihat,
                // dikalikan dengan _CloudOpacity untuk kontrol keseluruhan.
                col += cloudContribution * (cloudSample.a * _CloudOpacity);
                // --- Penambahan Awan Selesai ---
